...
export async function fetchDataSpans(): Promise<
  Array<DistroWatchRankingDataSpan>
> {
...
export function mapRankingType(
  rankingTypeText: string
): DistroWatchRankingType {
...
export async function fetchRanking(
  ...
  const rankingDistributionElements = $(
    "> tbody > tr:nth-child(1n+4)",
    rankingTableElement
  );
  ...
  return {
    ...
    distributionsRanking: rankingDistributionElements
      .toArray()
      .map(rankingDistributionElement => {
        return {
          rank: parseInt($(":nth-child(1)", rankingDistributionElement).text()),
          name: $(":nth-child(2)", rankingDistributionElement).text(),
          value: parseFloat(
            $(":nth-child(3)", rankingDistributionElement).text()
          )
        };
      })
  };
